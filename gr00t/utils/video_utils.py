...
def get_frames_by_indices(
    ...
    if video_backend == "torchcodec":
        ...
        if requested.ndim != 1 or np.any(requested < 0):
            raise ValueError("frame indices must be a one-dimensional non-negative array")
        if requested.size == 0:
            return np.empty((0,), dtype=np.uint8)

        needed = set(requested.tolist())
        ...
        if missing:
            raise ValueError(f"Unable to read frame indices {missing} from {video_path}")
        return np.stack([decoded[int(index)] for index in requested])
    else:
