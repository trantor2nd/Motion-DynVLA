...
LEROBOT_GR00T_STATS_FILE_NAME = "stats_gr00t.json"
...
class LeRobotEpisodeLoader:
    ...
    def __init__(
        ...
        if episode_indices is not None:
            ...
            metadata_by_index = {
                int(metadata["episode_index"]): metadata for metadata in self.episodes_metadata
            }
            missing = sorted(set(requested) - set(metadata_by_index))
            if missing:
                raise ValueError(f"episode_indices not found in dataset: {missing}")
            self.episodes_metadata = [metadata_by_index[index] for index in requested]
    ...
    def _load_metadata(self) -> None:
        ...
        if not stats_path.exists():
            stats_path = meta_dir / LEROBOT_GR00T_STATS_FILE_NAME
        assert stats_path.exists(), (
            f"neither {LEROBOT_STATS_FILE_NAME} nor {LEROBOT_GR00T_STATS_FILE_NAME} "
            f"exists for {self.dataset_path}; please use gr00t/data/stats.py to generate it"
        )
        with open(stats_path, "r") as f:
            statistics_payload = json.load(f)
            self.stats = statistics_payload.get("statistics", statistics_payload)
