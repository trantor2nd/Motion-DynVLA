...
class ShardedSingleStepDataset(ShardedDataset):
    ...
    def _is_recoverable_video_error(self, error: Exception) -> bool:
        """Return whether an episode failed only because its video is unreadable."""
        ...
        return (
            error_module == "av"
            or error_module.startswith("av.")
            or "unable to read frame" in error_message
            or "invalid data found when processing input" in error_message
        )
    ...
    def get_shard(self, idx: int) -> list:
        ...
        for ep_idx, step_indices in episodes:
            ...
            try:
                ...
                if not self._is_recoverable_video_error(error):
                    raise
                episode_metadata = self.episode_loader.episodes_metadata[ep_idx]
                episode_id = episode_metadata.get("episode_index", ep_idx)
                warnings.warn(
                    "Skipping unreadable video episode "
                    f"dataset={self.dataset_path} loader_index={ep_idx} "
                    f"episode_index={episode_id} error={type(error).__name__} {error}",
                    stacklevel=2,
                )
                continue
