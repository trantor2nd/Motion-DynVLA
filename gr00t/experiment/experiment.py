...
def run(config: Config):
    ...
    # Avoid duplicating the final multi-gigabyte checkpoint at the run root.
    if config.training.save_model_at_end:
        ...
        if last_checkpoint is None:
            ...
            temporary_link = output_dir / ".final.codex-tmp"
            if temporary_link.is_symlink():
                temporary_link.unlink()
            if final_link.exists() and not final_link.is_symlink():
                raise RuntimeError(f"refusing to replace non-symlink final path: {final_link}")
            temporary_link.symlink_to(Path(last_checkpoint).name, target_is_directory=True)
            os.replace(temporary_link, final_link)
            logging.info(f"Final checkpoint link: {final_link} -> {last_checkpoint}")
    else:
        logging.info("Final model save disabled by configuration")
