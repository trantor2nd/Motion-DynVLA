        ...
        *and* model outputs, we calculate accuracy and push it to the logger.
        ...
        if self.state.global_step % self.args.logging_steps == 0 and model.training:
            ...
            for key in (
                ...
                "temporal_loss",
                "identification_loss",
                "bank_commitment_loss",
                "bank_diversity_loss",
            ):
                value = outputs.get(key) if hasattr(outputs, "get") else None
                if torch.is_tensor(value) and value.numel() == 1:
                    gathered = self._nested_gather(value.detach()).float().mean().item()
                    dynamics_metrics[f"train_{key}"] = gathered
            if dynamics_metrics and self.args.local_rank in (-1, 0):
                self.log(dynamics_metrics)
