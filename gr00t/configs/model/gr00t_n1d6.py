...
class Gr00tN1d6Config(PretrainedConfig):
    ...
    # DynVLA motion dynamics extension. Disabled by default so the unmodified
    # GR00T N1.6 checkpoint keeps its exact upstream behavior.
    ...
    dynvla_bank_mode: str = "trainable"
    dynvla_bank_seed: int = 7
    dynvla_teacher_dropout: float = 0.5
    dynvla_mdm_hidden_dim: int = 384
    dynvla_mdm_layers: int = 2
    dynvla_mdm_dropout: float = 0.1
    dynvla_contrastive_dim: int = 256
    dynvla_contrastive_temperature: float = 0.1
    dynvla_ema_momentum: float = 0.99
    dynvla_lambda_temporal: float = 0.1
    dynvla_lambda_identification: float = 0.1
    dynvla_lambda_bank_commitment: float = 0.05
    dynvla_lambda_bank_diversity: float = 0.01
